...
$o[24] =~ s/(FBgn[0-9]+)/<a href=http:\/\/flybase\.bio\.indiana\.edu\/\.bin\/fbidq\.html?$1 target=_blank>$1<\/a>/g;
 	...
 	print "<SMALL>Accessions: $o[22] $o[23]\n",br; 
	$go=$o[30];
	$go=~ s/\/\/[^\/]+\/\/\//; /g;
	$go=~ s/ inferred [^\/]+//g;
	$go=~ s/\/\//\//g;
...
while (<HOMOPHILA>) {
   chomp;
   ($transcript,$omim,$refseq,$evalue,$gen,$descript,$disease)=split (/\t/,$_);
   $cgname=$transcript;
   $cgname=~ s/-P[A-Z]//;
   unless ($om =~ $omim) {
   $om.= $omim;
   if ($transcript =~ $cg) {
               print "<a href=http://superfly.ucsd.edu/homophila/ target=_blank>Homophila</a> reports that $cg is a close match ($evalue) to  human gene $gen (<a href=http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?db=gene&cmd=search&term=$refseq target=_blank>$refseq</a>), a gene associated with <a href=http://omim.org/entry/$omim target=_blank>$disease</a>.",br;
			 }
		}
   }
close HOMOPHILA;
...
#       print "$cg",br;
 ...
 #look for matching Vienna data 
      ...
      print "<font face=\"Verdana, Arial, Helvetica, sans-serif\"><table border=2 style=\"font-size: 11px\"><tr><td>Tissue</td><td>mRNA Signal</td><td>Present Call</td><td>Enrichment</td><td>Affy Call</td></tr>";
...
print "<a href=\'http://host-tracker.com/\' onMouseOver=\'this.href=\"http://host-tracker.com/website-monitoring-stats/3260065/lvuc/\";\'><img 
width=88 height=31 border=0 alt=\'measure downtime\' 
src=\"http://ext.host-tracker.com/uptime-img/?s=31&amp;t=3260065&amp;m=00.09&amp;p=Total&amp;src=lvuc\"></a><noscript><a href=\'http://host-tracker.com/\' >website testing</a></noscript>",br;
print end_html;
}